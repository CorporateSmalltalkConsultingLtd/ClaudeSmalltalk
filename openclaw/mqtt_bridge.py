#!/usr/bin/env python3
"""Smalltalk MQTT Bridge — talk to a live Smalltalk image via MQTT.

Communicates with a Smalltalk image running an LLMHandler over MQTT
through the Paho client interface.

The Smalltalk JsonObjectEncoder/Decoder uses a custom format:
...
DEFAULT_BROKER = "192.168.1.158"
DEFAULT_PORT = 1883
DEFAULT_USER = "Hudson"
DEFAULT_PASS = "manager"
DEFAULT_IMAGE_ID = "dev1"
DEFAULT_TIMEOUT = 15


def to_smalltalk_json(obj):
    """Convert a Python dict to Smalltalk JSON Dictionary format."""
    if isinstance(obj, dict):
        contents = []
        for k, v in obj.items():
            contents.append([k, to_smalltalk_json(v)])
        return {"Dictionary": {"contents": contents}}
    if isinstance(obj, list):
        return [to_smalltalk_json(x) for x in obj]
    return obj


def from_smalltalk_json(obj):
    """Convert Smalltalk JSON Dictionary format back to Python dict."""
    if isinstance(obj, dict):
        if "Dictionary" in obj:
            result = {}
            for pair in obj["Dictionary"]["contents"]:
                result[pair[0]] = from_smalltalk_json(pair[1])
            return result
        if "ByteArray" in obj:
            return bytes(obj["ByteArray"]["contents"]).decode("utf-8", "replace")
        return {k: from_smalltalk_json(v) for k, v in obj.items()}
    if isinstance(obj, list):
        return [from_smalltalk_json(x) for x in obj]
    return obj
...
class MqttBridge:
    ...
    def request(self, action, payload):
        """Send a request to the Smalltalk LLMHandler and return the parsed response."""
        self._event.clear()
        self._result = {}
        ...
        if self._event.wait(self.timeout):
            ...
            parsed = from_smalltalk_json(self._result)
            if "error" in parsed:
                return {"error": parsed["error"], "stack": parsed.get("stack", "")}
            return parsed
        return {"error": "timeout waiting for MQTT response"}
...
def main():
    parser = argparse.ArgumentParser(description="Smalltalk MQTT Bridge")
    ...
    p = sub.add_parser("check", help="Check MQTT connectivity")
    ...
    try:
        if args.command == "check":
            ...
            if "error" in r:
                ...
                sys.exit(1)
            print(f"OK — image responded: {r.get('result', r)}")
            sys.exit(0)
